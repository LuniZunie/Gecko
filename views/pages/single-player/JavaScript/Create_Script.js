const CreateGame = {
    ...
    Functions: {
        ...
        OnStartup: function () {
            CreateGame.gameDiv = document.getElementById("Create_Game");

            CreateGame.board = document.getElementById("Create_ChessBoard");

            CreateGame.whiteTimerElement = document.getElementById("Create_WhiteTimer");
            CreateGame.blackTimerElement = document.getElementById("Create_BlackTimer");
        ...
        SetupPosition: function () {//working on
            CreateGame.Position = structuredClone(CreateGame.StartPosition);

            CreateGame.whiteStarts = true;

            for (const y in CreateGame.Position) {
                if (CreateGame.Position[y] === [null, null, null, null, null, null, null, null])
                    continue;

                for (const x in CreateGame.Position[y]) {
                    const piece = CreateGame.Position[y][x];
                    if (piece) {
                        if (piece[2] === "0") {
                            if (piece[0] === "0")
                                CreateGame.whiteKingPosition = [x, y];
                            else
                                CreateGame.blackKingPosition = [x, y];
                        } else { //everything except king
                            CreateGame.material.total[`x${piece[2]}`]++;

                            if (piece[0] === "0")
                                CreateGame.material.white[`x${piece[2]}`]++;
                            else
                                CreateGame.material.black[`x${piece[2]}`]++;
                        }
                    }
                }
            }
